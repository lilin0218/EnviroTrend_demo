...
Rectangle {
    ...
    property var currentTimestampBuffer: core.timestampBuffer
    ...
    RowLayout {
        ...
        Item {
            ...
            TrendChart {
                ...
                timestampBuffer: root.currentTimestampBuffer
            }
        }
    }
        ...
        if (currentIndex >= 0 && currentIndex < sensorConfigs.count) {
            ...
            switch(config.bufferType) {
                case "temp":
                    ...
                    root.currentTimestampBuffer = core.timestampBuffer;
                    break;
                case "hum":
                ...
                default:
                    ...
                    root.currentTimestampBuffer = [];
            }

            // 如果当前没有预测数据且AI不忙，自动触发预测
            if (root.currentPredictList.length === 0 && !core.isAiBusy) {
                core.runPrediction()
            }
        }
    }

    // 监听core的buffer更新信号，重新加载当前数据
    Connections {
        target: core
        onSampledBufferUpdated: {
            // 刷新当前buffer
            var config = sensorConfigs.get(root.currentIndex);
            if (config) {
                switch(config.bufferType) {
                    case "temp":
                        root.currentDataBuffer = core.sampledTempBuffer;
