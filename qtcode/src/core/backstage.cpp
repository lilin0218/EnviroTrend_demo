...
Backstage::Backstage(QObject *parent)
    ...
    bool dbReady = false;
    if (!initStorage()) {
        Logger::instance()->warning("DB", "Storage init failed, will run without database");
        m_dbPath.clear();
    } else if (!initDatabase()) {
        Logger::instance()->warning("DB", "Database init failed, will run without database");
        emit errorOccurred("Failed to initialize database");
    } else if (!loadBufferFromDatabase()) {
        Logger::instance()->warning("DB", "Failed to load buffer from database");
    ...
    Logger::instance()->info("CORE", QString("Sample timer started, interval: %1ms, database: %2").arg(m_msPerPoint).arg(dbReady ? "ready" : "unavailable"));
}
...
bool Backstage::loadBufferFromDatabase() {
    ...
    QSqlQuery query(m_db);
    QString sql = QString(
        "SELECT timestamp, temperature, humidity, light, mq135, zp01 FROM sensor_data "
    ...
    while (query.next()) {
        // 获取时间戳
        QVariant timestampVal = query.value(0);
        QString timestampStr = timestampVal.toString();
        QDateTime dateTime = QDateTime::fromString(timestampStr, "yyyy-MM-dd HH:mm:ss");
        timestampList.prepend(dateTime.isValid() ? dateTime.toMSecsSinceEpoch() : QVariant(qQNaN()));

        // 使用 QVariant 保存，NULL值保持为无效的 QVariant
        QVariant tempVal = query.value(1);
        QVariant humVal = query.value(2);
        QVariant lightVal = query.value(3);
        QVariant mq135Val = query.value(4);
        QVariant zp01Val = query.value(5);
    ...
    m_timestampBuffer = timestampList;
...
bool Backstage::updateDatabase(double t, double h, double light, double mq135, double zp01) {
    ...
    Logger::instance()->info("DB", QString("Data written: temp=%1, hum=%2, light=%3, mq135=%4, zp01=%5").arg(t, 0, 'f', 1).arg(h, 0, 'f', 1).arg(light, 0, 'f', 3).arg(mq135, 0, 'f', 1).arg(zp01, 0, 'f', 1));
    
    // 删除超过1周的老数据（已禁用：不再自动删除历史数据）
...
void Backstage::processSnapshot() {
    ...
    m_timestampBuffer.append(QVariant(nowMs));
    if (m_timestampBuffer.size() > MAX_POINTS_24H) {
        m_timestampBuffer.removeFirst();
    }
    ...
    Logger::instance()->debug("CORE", QString("Snapshot taken, buffer size: %1, timestamp: %2").arg(m_tempBuffer.size()).arg(now.toString("HH:mm:ss")));
    emit bufferSig();
}
...
void Backstage::uploadToServer() {
    ...
    Logger::instance()->info("UPLOAD", QString("Uploading: temp=%1, hum=%2, light=%3, mq135=%4, zp01=%5")
        .arg(m_currentTemp, 0, 'f', 1).arg(m_currentHum, 0, 'f', 1)
        .arg(m_currentLight, 0, 'f', 3).arg(m_currentMQ135, 0, 'f', 1).arg(m_currentZP01, 0, 'f', 1));
...
bool Backstage::purgeOldData(int daysToKeep) {
    ...
    Logger::instance()->info("DB", "purgeOldData called but disabled");
    return true;
}
...
void Backstage::handleDHT11(const QVariantMap &data) {
    ...
    Logger::instance()->debug("SENSOR", QString("DHT11 measured: temp=%1°C, hum=%2%").arg(m_currentTemp, 0, 'f', 1).arg(m_currentHum, 0, 'f', 1));
    emit valueSig();
}
...
void Backstage::handleLight(const QVariantMap &data) {
    ...
    Logger::instance()->debug("SENSOR", QString("Light measured: %1V").arg(m_currentLight, 0, 'f', 3));
    emit valueSig();
}
...
void Backstage::handleZP(const QVariantMap &data) {
    ...
    Logger::instance()->debug("SENSOR", QString("ZP01 measured: %1μg/m³").arg(m_currentZP01, 0, 'f', 1));
    emit valueSig();
}
...
void Backstage::handleMQ135(const QVariantMap &data) {
    ...
    Logger::instance()->debug("SENSOR", QString("MQ135 measured: %1ppm").arg(m_currentMQ135, 0, 'f', 1));
    emit valueSig();
}
...
QVariantList Backstage::getTimestampBuffer() const {
    QVariantList result;
    for (int i = 0; i < m_timestampBuffer.size(); i += m_sampleStep) {
        result.append(m_timestampBuffer.at(i));
    }
    return result;
}
