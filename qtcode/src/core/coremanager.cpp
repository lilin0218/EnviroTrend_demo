    ...
    m_isAiBusy(false)
{
    ...
    QTimer::singleShot(500, this, [this]() {
        ...
        Logger::instance()->info("CORE", QString("Sensor mode: %1").arg(isHardware ? "hardware" : "mock"));
...
void CoreManager::handleProcessOutput() {
    ...
    if (rawOutput.isEmpty()) {
        Logger::instance()->warning("AI", "No prediction data received from script");
...
void CoreManager::onSampledBufferUpdated() {
    int tempSize = m_backstage->getSampledTempBuffer().size();
    Logger::instance()->info("QML", QString("Chart data updated, sampled points: %1, step: %2").arg(tempSize).arg(m_backstage->getSampleStep()));
...
QVariantList CoreManager::timestampBuffer() const {
    return m_backstage->getTimestampBuffer();
}
